...
class AlphaEstimator():
    def __init__(self, layer, device, shape_in, shape_out, x_min=None, x_max=None, deg=128):
        ...
        for i, x_min in enumerate(self.x_min):
            ...
            # # print(x_min, x_max)
            # a = min(x_min / (0.5 * x_max) - 1, 1)
            # b = 1
            # # print(a, b)
            # self.coeffs[2 * i + 1, 0] = 1.0 / pi * (acos(a) - acos(b)) * \
            #              (sin(1) * alphaP / ((self.deg + 2)*sin(alphaP)) + (1-(1)/(self.deg + 2)) * cos(0 * alphaP))
            # for j in range(1, deg):
            #     self.coeffs[2 * i + 1, j] = 2.0 / pi * (sin(j * acos(a)) - sin(j * acos(b))) / j * \
            #         (sin(j + 1)* alphaP / ((self.deg + 2) * sin(alphaP)) + (1 - (j + 1) / (self.deg + 2)) * cos(j * alphaP))
    ...
    def fit(self):
        alphas, summe, n = self.estimateAlpha()
        # print(n)
        npn = n.cpu().numpy()
        try:
